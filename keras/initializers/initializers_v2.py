    ...
    def _warn_reuse(self):
        if getattr(self, "_used", False):
            if getattr(self, "seed", None) is None:
                warnings.warn(
                    f"The initializer {self.__class__.__name__} is being "
                    "called multiple times, which will return identical values "
                    "each time (even if the initializer is unseeded). Please "
                    "update your code to avoid using the same initalizer "
